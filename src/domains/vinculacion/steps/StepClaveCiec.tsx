...
import { Card, CardBody, Button, Skeleton, CardHeader } from "@heroui/react";
...
export default function StepClaveCiec({ id, rfc, onAdvance }: Props) {
    const [videoLoaded, setVideoLoaded] = React.useState(false);
    const [opening, setOpening] = React.useState(false);

    React.useEffect(() => {
        // precarga script para UX más fluida
    ...
    const handleOpen = async () => {
        try {
            ...
            openSatws({
                onSuccess: async () => {
                    try {
                        // Avanza el estado en el backend
